...
<aside class="w-72 bg-[#1e3a8a] text-white flex flex-col p-6 h-screen shrink-0 rounded-r-[30px] shadow-[10px_0_30px_-5px_rgba(0,0,0,0.3)] z-10">
    ...
    <nav class="flex-1 overflow-y-auto pr-2">
        ...
        <div class="space-y-4"> <!-- Mengatur jarak agar lebih rapi -->
            ...
            <a href="transaksi_barang.php" 
               class="flex items-cente gap-4 px-6 py-3 rounded-full transition-all <?php echo ($current_page == 'transaksi_barang.php') ? 'bg-[#3b82f6] text-white shadow-lg shadow-blue-900/40 font-bold' : 'text-white hover:text-blue-200 hover:bg-white/5 font-medium'; ?>">
    ...
    <div class="mt-auto pt-4 flex justify-center">
        <button onclick="konfirmasiLogout()" class="flex items-center justify-center w-[70%] py-2 bg-[#be123c] hover:bg-red-700 text-white font-bold rounded-full shadow-lg shadow-black/20 transition-all active:scale-95">
...
<div id="modalLogout" class="fixed inset-0 z-50 hidden items-center justify-center p-4"
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
        ...
        <h3 class="text-[14px] font-bold text-slate-800 text-center mb-1">Keluar Sistem?</h3>
        <p class="text-[12px] text-slate-500 text-center mb-5">Apakah Anda yakin ingin melakukan logout?</p>
        <p class="text-[11px] text-slate-400 text-center mb-5">Anda akan perlu login kembali untuk mengakses sistem.</p>
        <div class="flex gap-2">
            ...
            <a href="logout.php"
               class="flex-1 py-2 text-[12px] font-bold text-white bg-[#be123c] rounded-xl hover:bg-red-700 transition text-center">
                Ya, Logout
            </a>
        </div>
    </div>
